...
def build_season(scenario: SeasonScenario = DEFAULT_SCENARIO) -> tuple[State, Registry]:
    ...
    on the well-fed run), ``soil_water``/``soil_n``/``plant_n`` POOLs, the unclamped
    ``co2_atmos``/``water_source``/``n_source`` boundary sources, the ``co2_resp``/
    ``vapor_sink``/``litter_sink`` boundary sinks, plus the carbon loss-sink for
    extinction routing (#6). Flows in canonical id order; the aux process is a
    registry construction dependency (advances ``State.aux``, P2).
    """
