...
| **CO₂ enrichment**, priced on the shipped integrator (the headroom diagnosis's successor 3) | **DIAGNOSED 2026-08-13, NOTHING BUILT** — the recorded "a fifth of the headroom per doubling" was a chord through three points; the real sweep is **non-monotone** and rations only at 1785 ppm, above where a realism move lands. The finding is elsewhere: the crop **cannot** draw the chamber below the CO₂ compensation point (`Γ*/ci_ratio = 61.07 ppm`), and RK4 pins the season low at **~76 ppm at every enrichment level** — while shipped Euler at `dt = 1` **collapses it 57.9 → 12.1 ppm**, fixing carbon where the model says it fixes none. ⚠ Unlike the margin, this **converges** rather than scaling with `dt` (57.9 → 75.1 → 75.8), so it is a statement about the *answer*, not the guard. ⚠ The **frozen ambient** run already crosses the floor (24 % low) and **overstates** the enrichment payoff by 4.3 %. Verdict: *enrichment fails scientifically ~600 ppm before it fails numerically, and the margin is the wrong alarm* | `post-roadmap-co2-enrichment-margin.md` |
