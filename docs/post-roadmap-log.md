...
| **The CO₂ setpoint controller, priced** (the direction plan's Step 0, axis 2 — the axis that could have cancelled the step gate) | **DIAGNOSED 2026-08-13, NOTHING BUILT** — probe only, two controls first (the no-op clamp is bit-identical to `run_perennial` on all three chambers; the live clamp migrates the binding day). ⚠⚠ **The ambient control inverted the hypothesis**: holding the sealed chamber at the 357 ppm it already starts from is **four times worse** than letting it deplete (margin 1.3072 → 0.2977, season-low CO₂ 57.9 → 10.6 ppm, rationing 0 → 270), because the uncontrolled chamber **self-limits** and a controller removes that feedback — the argument needed the stock to be *large*, not *steady*. **No realistic setpoint clears**: 1000–1200 ppm sits at margin 0.41–0.51 with 210–252 silent firings and season-low CO₂ ~75 % *below* the compensation point; the first pass is ~3000 ppm. And the discriminator — a convergence check **within** the setpoint, because the case against the step is a truncation error, not a threshold crossing — says **the controller doubles the step price**: controlled `dt = 1` at 1200 ppm is **22 % low on peak leaf carbon against its own `dt = ⅛`** (frozen: 3.2 %), and where the frozen tree clears at `dt = ½`, the controlled tree needs `dt = ¼`. ⚠ It also makes **RK4 at `dt = 1` unbuildable** (hard error at every setpoint), and generalizes past the probe: arbitration credits **no same-step inflows**, so a make-up *flow* is strictly worse than this dawn clamp, which is therefore the **upper bound on any controller**. Verdict: the *"may dissolve this whole gate"* warning is **refuted**, the conditional `dt = ½` recommendation is **discharged and stands**, and the controller moves back behind the gate as a `dt = ¼` object | `post-roadmap-co2-controller.md` |
