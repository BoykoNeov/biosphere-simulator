...
| Re-deriving the soil water store from geometry | DIAGNOSED 2026-08-12, **NOT BUILT** — the recorded price ("every frozen scenario water-stressed") is wrong both ways: the open field costs −2 % yield, and **every sealed chamber dies and locks**, missing the escape by 0.5 kg. The geometry and the `FTSW` successor are **one mechanism**; together they move only the two water stocks on 4 of 5 scenarios. Scope is the user's | `post-roadmap-soil-water-rebasing.md` |
...
| **Re-deriving the soil water store from geometry** (the soil-layers build's own named finding, taken) | [the record](log/soil-water-rebasing.md) |
