...
**1. Eighteen of the twenty-five goldens are now the Rust port's own bytes.** They are
regenerated by `tests/crossport/regen_goldens_from_rust.py --write`, and the Python
`_regenerate()` mains refuse to write them (`tests/golden_platform.write_python_golden`).
The remaining seven stay Python-authored, each for a stated reason — two folded summaries
where *the fold is the artifact*, four scenarios Rust has no equivalent of, and one
hand-authored fixture Rust *reads*.
