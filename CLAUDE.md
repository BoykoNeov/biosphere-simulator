...
| Bucket 2 (cont.): **multi-rate authoring** — the chosen fix | **IN PROGRESS — Steps 1–2 of 7 done; the authoring platform is UNFROZEN and the manifest has moved.** `docs/plans/post-roadmap-multirate-authoring.md`. An author picks a **coupling cadence**, not a global `dt`; each flow sub-steps at the `dt` its own rate constant demands. **Step 1** pinned the fact the phase rests on: `n_sub=1` + an empty slow set reproduces the frozen ECLSS golden **byte-for-byte**, so no golden need move — measured, not inferred. **Step 2** landed the knob (`ScenarioSpec.n_sub`, `FlowSpec.rate_class`, the disjoint slow/fast registries on `BuiltScenario`); it **builds** the partition but nothing drives it yet (Step 3). Two rulings worth carrying: the partition check lives in the **interpreter, not a pydantic validator**, because a *bundle* may contribute `rate_class: slow` and a schema-level check runs before `apply_includes` would ever see it; and the **manifest regenerates at each step that moves the surface**, never batched to a final ceremony — the gate is plain equality, so batching does not save the ceremony, it disables the gate for the steps in between. Resolves the composability constraint the reference calls **unsolvable** ("there is no `dt` natural to both domains" — with multi-rate there is: master 3600, ECLSS fast at `n_sub=60`). **`simcore.multirate` already exists and is proven** (Phase 0.5) — this is a *consumer* phase; `git diff src/simcore/` must stay empty. Rejected alternatives, priced: the **implicit integrator** (the instinctive "rework the maths") is the repo's largest unfreeze **and does not meet the bar** — backward Euler gives 10.61 vs truth 8.33 at `dt=1800`; it buys *sane*, not *near*. **Multi-rate is the performance enabler, NOT the hazard closer** (advisor): an unsafe **effective sub-step** `dt/n_sub` is the same hazard one level down, so the deferred build-time precondition must check `dt/n_sub`, never the master `dt` |
