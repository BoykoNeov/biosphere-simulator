...
| Root functional coupling (the wheat refusal's successor) | TAKEN; the **NITROGEN half REFUSED 2026-08-11** (the water half is untested, not refuted) — [E] p. 136 decouples rooted depth from root mass **on purpose**; a depth gate on N is bit-identical on the whole frozen roster incl. both 15-yr runs. Uptake is demand-bound everywhere, so **no supply-side root coupling can bite**. The last uncited N param is citable but its band is scientifically inert | `post-roadmap-root-functional-coupling.md` |
