...
from day_index import by_day, days_in
...
def test_gap_allocation_is_root_heavy() -> None:
    ...
    states = _season_states()
    ref = _reference()
    n = min(len(ref), days_in(states))  # was `len(states)` — a STEP count
    days = by_day(states, n)
...
def test_method_matched_day_confound_dissolved_with_the_phenology_fix() -> None:
    ...
    *after* anthesis, and ``StemRemobilization`` has a lower DVS trigger with **no upper
    ...
    n_off = min(len(ref), days_in(off))  # was `len(off)` — a STEP count
    off_days = by_day(off, n_off)
...
def test_method_the_death_spiral_is_gone() -> None:
    ...
    leaf = [s.stocks[LEAF_C].amount for s in by_day(states, len(_reference()))]
