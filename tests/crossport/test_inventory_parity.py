...
def test_rust_inventory_equals_the_frozen_manifest(
    ...
    # ⚠ De-branched in C7's station half, exactly as the comment here scheduled: "when
    # the station writer lands this collapses back to one string". Both manifests are
    # regenerated by their own Rust example now, so the advice differs only in which
    # example to name — and printing the wrong command is the "correct assertion, wrong
    # advice" failure slice 5 recorded.
    example = "dump_biosphere_inventory" if biosphere else "dump_station_inventory"
    regen = f"cd rust && cargo run --example {example} -- --write-manifest"
    selection = (
        ""
        if biosphere
        else (
...
# ⚠⚠ **`test_the_dump_key_sets_are_the_ones_the_generators_consume` RETIRED WITH THE
# STATION WRITER (C7's station half), exactly as its own docstring scheduled.** It tied
# each dump's key set — declared once in this module for the staleness gate and once
# in the generator that refused an unclassified key — so the two copies could not
# drift. The biosphere row left in C7's biosphere half, the authoring row in its
# authoring half;
# the station row was the last, and it leaves for the same narrow reason:
#
#   **there is no second copy of the key set left — the copy lived in the writer.**
#
# ⚠ That is deliberately narrower than the reason first written when the biosphere row
# left ("nothing consumes the dump"), which the authoring case then proved false — that
# dump is still consumed. All three dumps are still consumed, here, by
# `test_rust_inventory_equals_the_frozen_manifest`. What ended is the *duplication*, not
# the consumption.
#
# What replaced the forcing function is named rather than assumed: `set(dump) ==
# dump_keys` in the staleness gate above still stops an unclassified dump key from
# entering unnoticed, and `tests/crossport/test_manifest_writer.py` compares each whole
# committed manifest against a regeneration — which catches the case this test never
# could, a hand edit to the committed file.
