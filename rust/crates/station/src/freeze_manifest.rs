...
// ⚠⚠ **RELOCATED from `examples/dump_station_inventory.rs` by Stage-3 slice S2** (the reference flip,
...
use crate::scenario::{sealed_station_scenario, SEALED_ENERGY_YEARS, SEALED_STATION_YEARS};
use crate::science_gates::GATES;
use crate::sealed::build_sealed_station;
...
pub fn committed_manifest_path() -> PathBuf {
    repo_root().join("docs").join("station-reference.manifest.json")
}
