...
/// The `dump_station_inventory` program, as text. Same reason as `WRITER_SOURCE` above: a renamed or
/// moved example is a compile error here rather than a test that quietly reads nothing.
const EXAMPLE_SOURCE: &str = include_str!("../examples/dump_station_inventory.rs");
...
fn the_default_write_target_is_the_contract_this_gate_compares() {
    let target = station::freeze_manifest::committed_manifest_path();
    assert!(
        target.is_file(),
...
/// `--write-manifest`, which calls [`station::freeze_manifest::write_manifest`]. Those were
...
fn the_write_path_produces_exactly_what_the_byte_gate_compares() {
    let dir = std::path::PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("manifest_write_station");
    std::fs::create_dir_all(&dir).expect("the temp dir is creatable");
    let target = dir.join("regenerated.manifest.json");
    station::freeze_manifest::write_manifest(&target);
    let written = std::fs::read(&target).expect("the writer wrote a file");
    assert_eq!(
        written,
        station::freeze_manifest::manifest_text().into_bytes(),
