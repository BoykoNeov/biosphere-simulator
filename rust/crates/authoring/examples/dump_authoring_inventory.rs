...
use authoring::freeze_manifest::{committed_manifest_path, dump, write_manifest};
...
/// default `--write-manifest` target lives in `authoring::freeze_manifest`, where this
...
fn main() {
    ...
    match parse_args("dump_authoring_inventory", &committed_manifest_path(), &args) {
