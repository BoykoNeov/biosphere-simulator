...
//! `domains::freeze_manifest` since Stage-3 slice S2 — an `examples/` binary cannot be
...
use domains::freeze_manifest::{committed_manifest_path, dump, write_manifest};
...
/// default `--write-manifest` target lives in `domains::freeze_manifest`, where this
...
fn main() {
    ...
    match parse_args("dump_biosphere_inventory", &committed_manifest_path(), &args) {
